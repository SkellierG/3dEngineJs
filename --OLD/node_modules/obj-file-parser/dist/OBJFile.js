...
let OBJFile = function () {
  ...
  _createClass(OBJFile, [{
    ...
    value: function parse() {
      ...
      let _stripComments = function _stripComments(lineString) {
      ...
      for (let i = 0; i < lines.length; i += 1) {
        let line = _stripComments(lines[i]);

        let lineItems = line.replace(/\s+/g, ' ').trim().split(' ');
    ...
    value: function _createNewModel() {
      let name = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.defaultModelName;
    ...
    value: function _currentModel() {
      if (this.result.models.length == 0) {
        let defaultModel = this._createNewModel();
    ...
    value: function _parseObject(lineItems) {
      let modelName = lineItems.length >= 2 ? lineItems[1] : this.defaultModelName;
      let model = this._createNewModel(modelName);
    ...
    value: function _parseVertexCoords(lineItems) {
      ...
      this._currentModel().vertices.push(vec3(x, y, z, 1));
    }
  }, {
    ...
    value: function _parseTextureCoords(lineItems) {
      ...
      this._currentModel().textureCoords.push(vec3(u, v, w, 1));
    }
  }, {
    ...
    value: function _parseVertexNormal(lineItems) {
      ...
      this._currentModel().vertexNormals.push(vec3(x, y, x, 1));
    }
  }, {
    key: '_parseLine',
    ...
    value: function _parseSmoothShadingStatement(lineItems) {
      ...
      let groupNumber = lineItems[1].toLowerCase() == 'off' ? 0 : parseInt(lineItems[1]);
